...
split <- sample.split(slack.lab.data, SplitRatio = 0.7)
split

train <- subset(slack.lab.data, split == "TRUE")
test <- subset(slack.lab.data, split == "FALSE")


# Fitting Random Forest to the train dataset
set.seed(1003)  # Setting seed
classifier_RF = randomForest(x = train[-81],
                             y = train$isVPN,
                             ntree = 500)
...
y_pred = predict(classifier_RF, newdata = test[-81])
y_pred.res = predict(classifier_RF, newdata = slack.res.data[-81])

# Confusion Matrix
confusion_mtx = confusionMatrix(y_pred, test$isVPN)
confusion_mtx.res = confusionMatrix(y_pred.res, slack.res.data$isVPN)
confusion_mtx
confusion_mtx.res
```